...
public class SwipeBackLayout extends FrameLayout {
    ...
    /**
     *the SwipeBackLayout will attach to Activity
     */
     ...
     * callback's
     * {@link com.huoqiu.framework.SwipeBackLayout.ViewDragHelper.Callback#onEdgeTouched(int, int)}
     * and
     * {@link com.huoqiu.framework.SwipeBackLayout.ViewDragHelper.Callback#onEdgeDragStarted(int, int)}
    ...
    public void attachToFragment(Fragment fragment) {
        ...
        mFrament = fragment;
        TypedArray a = mFrament.getActivity().getTheme().obtainStyledAttributes(new int[] {
                android.R.attr.windowBackground
        });
        int background = a.getResourceId(0, 0);
        a.recycle();
...
//            decor.setBackground(new ColorDrawable(Color.TRANSPARENT));
//        } else {
//            decor.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
//        }
        decor.removeView(decorChild);
        addView(decorChild);
        setContentView(decorChild);
    ...
    private class ViewDragCallback extends ViewDragHelper.Callback {
        ...
        private boolean mIsHaveRemoved = false;
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if (mScrollPercent >= 1 && !mIsHaveRemoved) {
                mIsHaveRemoved = true;
                if (getAttchMode() == AttachToActivity){
                    if (!mActivity.isFinishing()){
                        mActivity.finish();
                    }
                }else if(getAttchMode() == AttachToFragment){
                    if(mFrament instanceof SuperFragment){
                        ((SuperFragment)mFrament).remove();
                    }else{
                        mFrament.getActivity().onBackPressed();
                    }
                }
